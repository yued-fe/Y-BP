...
1. 按钮主题 「 _theme.scss 」：按钮主题其实是按照功能分，只是设计师通常用颜色区分功能，所以主题也近似可以看作是颜色；
2. 按钮状态 「 _status.scss 」：按钮处于一些临界点的时候需要有一些特殊的状样式告知用户，比如说不可用的状态，或者鼠标按下的状态；
3. 按钮大小 「 _size.scss 」：按钮尺寸的区别；
4. 按钮形状 「 _shape.scss 」：是否有背景，是否有边框，是否有圆角...;
